...
# TODO: Refactor into Bayes function which does the generative model inversion
...
class ExactLinearAgent:
    def __init__(self, params: dict, uniform_prior: bool = False) -> None:
        self.params = SimpleNamespace(**params)
    ...
    def build(self, y: Union[float, np.ndarray]):
        ...
        # self.likelihood, self.prior = GenerativeModels.gaussian(
        #     y=y, x=self.params.x_range,
        #     mu_y = self.generating_function, std_y=self.params.std_y,
        #     m_x=self.params.m_x, s_x=self.params.s_x)
